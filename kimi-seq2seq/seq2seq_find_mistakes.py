"""
Find Prediction Mistakes on Full Dataset
=========================================

Loads best_model.pt, runs inference on the entire dataset,
and saves all incorrect predictions to a CSV file.
...
def main():
    ...
    parser.add_argument('--output', type=str, default='mistakes.csv')
    ...
    print(f"Running inference on all {len(df)} words (batch_size={args.batch_size})...")
    ...
    if mistakes:
        ...
        print(f"\nSaved {len(mistakes)} mistakes to: {args.output}")
    else:
